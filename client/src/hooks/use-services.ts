...
export function useMobileInfo() {
  ...
  return useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: [api.user.history.path] });
    },
    onError: (error: Error) => {
...
export function useEmailInfo() {
  ...
  return useMutation({
    mutationFn: async (data: z.infer<typeof api.services.email.input>) => {
      const token = (window as any).firebaseToken;
      const res = await fetch(api.services.email.path, {
        method: api.services.email.method,
        headers: {
          "Content-Type": "application/json",
          ...(token ? { "Authorization": `Bearer ${token}` } : {}),
        },
      ...
      return api.services.email.responses[200].parse(await res.json());
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: [api.user.me.path] });
      queryClient.invalidateQueries({ queryKey: [api.user.history.path] });
    ...
    onError: (error: Error) => {
      if (error.message.toLowerCase().includes("protected")) return;
      toast({
        title: "SYSTEM ERROR",
